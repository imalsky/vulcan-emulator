...
## Planned next — hybrid arch retrain
**Config change**: revert sizing + wd to baseline values (d_model=256, L=6,
dim_ff=1024, cond=256, divisor=2, wd=1e-4). Keep sweep-era formulation wins
(norm=layernorm, ffn=swiglu, qk_norm=on, zero_init_film=on). ema stays on.
**Expectation**: val ≤ 0.0304 → formulation wins are neutral-or-good, we keep
them. val > 0.0304 → strip formulation changes back to rmsnorm/dense defaults
(exact baseline).

## Sweep v2 (to follow hybrid run)
...
- Search space narrows: silu / swiglu / qk_norm / zero_init_film fixed
  (sweep v1 showed these are consistently good); d_model ∈ {192, 256, 384};
  L ∈ [4, 8]; output_head_divisor ∈ {1, 2}; wd widened to [1e-6, 5e-3];
  new: `ema_enabled ∈ {true, false}`.
- Incumbent arch auto-enqueued as trial #0 (`INCUMBENT_PARAMS`) so every sweep
  has an apples-to-apples benchmark.
