...
from src.models.exojax_api import make_equilibrium_vmr_fn, make_full_vulcan_vmr_fn
...
def _ratio_globals() -> dict[str, float]:
    """Return one chemistry setting in the exported bundle ratio-global order."""
    return {
...
def _make_full_vulcan_bundle(tmp_path):
    ...
    normalization = {
        ...
        "global_static": {
            ...
            "floor": [1e-30, None, None, None] + [None] * (len(FULL_GLOBAL_ORDER) - 4),
        },
        "spectrum": {
            "method": "log-standard",
...
def _make_legacy_full_vulcan_bundle(tmp_path):
    """Build a legacy elemental-conditioning full-VULCAN bundle for migration checks."""
    dims = ModelDimensions(
        sequence_dim=3,
        global_dim=6,
        spectrum_dim=4,
    ...
    params = init_model_params(jax.random.PRNGKey(9), dims)
    payload = {
        "params": jax.tree_util.tree_map(np.asarray, params),
        "model_dimensions": dims.to_dict(),
        "normalization": {
    ...
    bundle_path = export_checkpoint_payload(payload, tmp_path / "legacy_fv_bundle.npz")
    return load_exported_model(bundle_path)


def _full_vulcan_global_dict(bundle) -> dict[str, float]:
    """Return one full-VULCAN conditioning dict matching the exported bundle order."""
    return {
        "gravity_cm_s2": 900.0,
        **_ratio_globals(),
        **static_conditioning_defaults(bundle.config),
    }
...
def test_equilibrium_vmr_fn_output_shape_and_labels(tmp_path):
    ...
    vmr = vmr_fn(public_t, public_p, 0.0, 0.67, 0.03, gravity)
...
def test_equilibrium_vmr_fn_matches_bundle_after_level_reversal(tmp_path):
    ...
    vmr_api = np.asarray(vmr_fn(public_t, public_p, 0.0, 0.67, 0.03, gravity))
    vmr_ref = np.asarray(
        bundle.predict_equilibrium_profile(
            pressure_bar=internal_p,
            temperature_k=internal_t,
            global_inputs=_ratio_globals(),
        )
    )[::-1]
...
def test_equilibrium_vmr_fn_jit_and_vjp(tmp_path):
    ...
    eager = vmr_fn(T, P, 0.0, 0.67, 0.03, gravity)
    compiled = jax.jit(vmr_fn)(T, P, 0.0, 0.67, 0.03, gravity)
    np.testing.assert_allclose(np.asarray(eager), np.asarray(compiled), atol=1e-6)

    grad_t = jax.grad(lambda temp: jnp.sum(vmr_fn(temp, P, 0.0, 0.67, 0.03, gravity)))(T)
    assert grad_t.shape == T.shape

    vmr_val, vjp_fn = jax.vjp(vmr_fn, T, P, 0.0, 0.67, 0.03, gravity)
    g_t, g_p, g_met, g_c, g_s, g_g = vjp_fn(jnp.ones_like(vmr_val))
    ...
    batched_met = jnp.asarray([0.0, 0.1], dtype=jnp.float32)
    batched_c = jnp.asarray([0.67, 0.72], dtype=jnp.float32)
    batched_s = jnp.asarray([0.03, 0.025], dtype=jnp.float32)
...
def test_full_vulcan_vmr_fn_matches_bundle_after_level_reversal(tmp_path):
    ...
    vmr_ref = np.asarray(
        bundle.predict_full_vulcan_profile(
            ...
            global_inputs=_full_vulcan_global_dict(bundle),
...
def test_full_vulcan_vmr_fn_jit_and_vjp(tmp_path):
    ...
    eager = vmr_fn(T, P, Kzz, 0.0, 0.67, 0.03, gravity, spectrum_flux)
    compiled = jax.jit(vmr_fn)(T, P, Kzz, 0.0, 0.67, 0.03, gravity, spectrum_flux)
    np.testing.assert_allclose(np.asarray(eager), np.asarray(compiled), atol=1e-6)

    grad_t = jax.grad(
        lambda temp: jnp.sum(vmr_fn(temp, P, Kzz, 0.0, 0.67, 0.03, gravity, spectrum_flux))
    )(T)
    assert grad_t.shape == T.shape
    ...
    g_t, g_p, g_kzz, g_met, g_c, g_s, g_g, g_spectrum = vjp_fn(jnp.ones_like(vmr_val))
    assert g_t.shape == T.shape
    assert g_p.shape == P.shape
...
def test_full_vulcan_vmr_fn_rejects_nonconstant_gravity_in_eager_mode(tmp_path):
    ...
    with pytest.raises(ValueError, match="gravity_cm_s2"):
        vmr_fn(T, P, Kzz, 0.0, 0.67, 0.03, gravity, spectrum_flux)


def test_full_vulcan_wrapper_rejects_legacy_elemental_bundle(tmp_path):
    legacy_bundle = _make_legacy_full_vulcan_bundle(tmp_path)

    with pytest.raises(ValueError, match="ratio-global chemistry contract"):
        make_full_vulcan_vmr_fn(legacy_bundle)
