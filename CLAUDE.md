...
### GPU-batched VULCAN-JAX generation (`generation.gpu_batch.enabled`)

For `backend == "vulcan_jax"` on a GPU node, set `generation.gpu_batch.enabled: true` to replace the per-profile subprocess pool with an **in-process, vmap-across-profiles** path (`_run_batch_vulcan_jax_gpu`): profiles are bucketed by `(nz, toggle-combo, atm_base)` and each bucket integrates in one `jax.vmap`'d call on the GPU via `vulcan_jax.OuterLoop.run_batch` (see VULCAN-JAX's CLAUDE.md). It writes the **same** per-run HDF5 contract into `runs_dir`, so the chunk-merge / resume / shard machinery downstream is unchanged. `_write_batched_run_hdf5` reuses `write_raw_run_hdf5`, reading the converged `ymix` from the unstacked `JaxIntegState` instead of a `.vul` pickle. The network is pinned via `$VULCAN_JAX_NETWORK` **before** the first `vulcan_jax` import (it's parsed once at import). Non-converged / non-finite runs go to backfill (`gpu_batch.require_converged`). The `run_gen.{pbs,sh}` / `run_gen_array.sh` scripts branch on this flag to use the GPU XLA env + a `nvidia-smi`/JAX-backend preflight. **Requires `vulcan-jax >= 0.1.10`.**

**Parallel host setup (the FastChem bottleneck fix).** Each profile's RunState (atmosphere + rates + FastChem-EQ abundances) is built on the host, dominated by the FastChem subprocess (~0.5–5 s/profile) — done serially it starves the GPU. Phase 1 fans this out across the Grace cores with a **persistent spawn `ProcessPool`** (`_host_setup_worker_*`, `_get_host_setup_pool`): each worker holds its own `vulcan_jax` import + a private FastChem tree (via `$VULCAN_JAX_FASTCHEM_DIR`, new in 0.1.10) so the cross-process flock is uncontended, builds the RunState with `with_pre_loop_setup(skip_chem_warmup=True)`, and pickles it back. **Processes not threads** because `vulcan_jax.state` reads the global `vulcan_cfg` (each spawn worker gets its own copy); **spawn not fork** because the parent has CUDA initialised. Phase 2 (parent) sets the bucket-representative cfg once, builds the device runner, `prepare_runstate`s each worker RunState, and integrates in `batch_size` chunks. Pool size = `gpu_batch.host_setup_workers` (`0` = auto-detect cores; `$VULCAN_GEN_HOST_SETUP_WORKERS` / `GEN_HOST_SETUP_WORKERS` overrides). `parallel_workers` is ignored in this mode. Validated end-to-end on CPU — the pool builds RunStates **bit-identical** to a sequential build (vmap is backend-agnostic); GPU throughput vs the 72-core path still needs to be benchmarked on the GH200.
