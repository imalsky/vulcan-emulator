...
For `backend == "vulcan_jax"` on a GPU node, set `generation.gpu_batch.enabled: true` to replace the per-profile subprocess pool with an **in-process, vmap-across-profiles** path (`_run_batch_vulcan_jax_gpu`): profiles are bucketed by `(nz, toggle-combo, atm_base)` and each bucket integrates in one `jax.vmap`'d call on the GPU via `vulcan_jax.OuterLoop.run_batch` (see VULCAN-JAX's CLAUDE.md). It writes the **same** per-run HDF5 contract into `runs_dir`, so the chunk-merge / resume / shard machinery downstream is unchanged. `_write_batched_run_hdf5` reuses `write_raw_run_hdf5`, reading the converged `ymix` from the unstacked `JaxIntegState` instead of a `.vul` pickle. This path is **single-process** (host RunState construction is sequential because `vulcan_jax.state` reads the global `vulcan_cfg` module; the GPU parallelises the integration), so `parallel_workers` is ignored when it's on. The network is pinned via `$VULCAN_JAX_NETWORK` **before** the first `vulcan_jax` import (it's parsed once at import). Non-converged / non-finite runs go to backfill (`gpu_batch.require_converged`). The `run_gen.{pbs,sh}` / `run_gen_array.sh` scripts branch on this flag to use the GPU XLA env + a `nvidia-smi`/JAX-backend preflight. Validated end-to-end on CPU (vmap is backend-agnostic); GPU throughput vs the 72-core path still needs to be benchmarked on the GH200.
